...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    elif schedule_name == "ddim":
        scheduler = DDIMScheduler(num_train_timesteps=num_diffusion_timesteps)
...
class GaussianDiffusion:
    ...
    def ddpm_sample(
        ...
        x0 = (1/ th.sqrt(alpha_bar)) * x -  th.sqrt(1/alpha_bar-1)* eps
	 
        mean_x0_coef = self.posterior_mean_coef1 
        mean_xt_coef = self.posterior_mean_coef2
        mean = mean_x0_coef * x0 + mean_xt_coef * x
        log_var = self.posterior_log_variance_clipped #zero at the begenning
        ...
        sample = mean + nonzero_mask * noise * (0.5 * log_var).exp()  
        
        return {"sample": sample, "pred_xstart": x0}
